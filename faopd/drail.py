...
try:
    ...
    pass
from faopd.ddpm import MLPConditionDiffusion
...
class DRAILDiscrim(BaseIRLAlgo):
    ...
    def plot_reward_map(self, i):
        if str(getattr(self.args, 'env_name', '')).startswith('Circle'):
        ...
        plt.imshow(reward, extent=extent, cmap="jet", origin="lower", aspect="auto")
        plt.colorbar()
    ...
    def plot_disc_val_map(self, i):
        ...
        with torch.no_grad():
            s = self._compute_disc_val(X, Y)
            eps = 1e-20
            logit = (s + eps).log() - (1 - s + eps).log()
            viz_map = logit.view(H, W).cpu().numpy().T
        plt.figure(figsize=(8, 5))
        plt.imshow(viz_map, extent=extent, cmap="jet", origin="lower", aspect="auto")
        plt.colorbar()
    ...
    def _update_reward_func(self, storage, gradient_clip=False, t=1):
        ...
        for epoch_i in range(self.args.n_drail_epochs):
            for expert_batch, agent_batch in zip(expert_sampler, agent_sampler):
                ...
                bc_coef = getattr(self.args, 'disc_bc_coef', 1.0)
                if bc_coef > 0.0 and hasattr(self.discrim_net, 'compute_bc_loss'):
                    
                    bc_loss = self.discrim_net.compute_bc_loss(
                        state= self._norm_expert_state(expert_batch['state'], obsfilt),
                        action= rutils.get_ac_repr(self.action_space, expert_batch['actions'].to(self.args.device)))
                    discrim_loss = discrim_loss + bc_coef * bc_loss
                    log_vals['disc_bc_loss'] += float(bc_loss)
        ...
        if (str(getattr(self.args, 'env_name', '')).startswith('Sine') or str(getattr(self.args, 'env_name', '')).startswith('Circle')):    
    ...
    def get_add_args(self, parser):
        ...
        parser.add_argument('--reward-type', type=str, default='airl', help="""
                ...
                not change training.
                """)
        # Discriminator-side FM-BC distillation (c=1, x=[s,a_expert])
        parser.add_argument('--disc-bc-coef', type=float, default=1.0,
            help='Applied on discriminator (0.0 disables)')
