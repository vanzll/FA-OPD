...
class DiffPolicy(BaseILAlgo):
    ...
    def _post_training_evaluation(self):
        if not hasattr(self, '_eval_policy') or not hasattr(self, '_log'):
            print("Evaluation function not available; skipping performance evaluation")
            return
        ...
        try:
            ...
            print(f"DP performance evaluation failed: {e}")
            final_metrics = {
            ...
            try:
                ...
                print("Unable to record final DP training status")
