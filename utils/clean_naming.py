...
def extract_subfolder_from_config(args) -> str:
    ...
    if not config_path:
        ...
        if env_name == 'maze' and hasattr(args, 'traj_load_path'):
            ...
            print(f"{env_name} environment: extracting coverage from traj_load_path: {traj_path}")
            import re
        ...
        if not config_path:
            print(f"{env_name} environment: cannot infer subfolder, using default")
            return 'default'
    ...
    try:
        ...
        print(f"Parse config path failed: {e}")
...
if __name__ == "__main__":
    ...
    print("Testing clean naming tool...")
